...
class DoSubscriptionController extends Controller
{
    ...
    public function showSubscriptionForm(Request $request)
    {
        ...
        try {
            // Utiliser config() au lieu de env() pour éviter les problèmes de cache
            ...
            if (!$api_username || !$api_password || !$api_url || !$api_secret || !$api_key) {
                Log::error('SUBSCRIPTION: Configuration API Musoni manquante (getCustomerMusoni)', [
            ...
            Log::error('SUBSCRIPTION: Erreur lors de la récupération des comptes client depuis Musoni', [
                'username' => session('username'),
                'client_id' => session('client_id'),
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
    ...
    public function getCustomerMusoni(Request $request)
    {
        ...
        // Vérifier que toutes les variables sont définies
        if (!$api_username || !$api_password || !$api_url || !$api_secret || !$api_key) {
            Log::error('SUBSCRIPTION: Configuration API Musoni manquante (sendValidationRequest)', [
                'username' => session('username'),
        ...
        try {
            ...
            if ($response->successful()) {
                ...
                try {
                    ...
                    Log::error('SUBSCRIPTION: Erreur lors de la récupération des informations client depuis Musoni', [
                        'username' => session('username'),
            ...
            Log::error('SUBSCRIPTION: Erreur lors de la recherche du client par CIN', [
                'username' => session('username'),
                'cin' => $request->input('cin'),
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
    ...
    public function sendValidationRequest(Request $request)
    {
        ...
        # __Déterminer le nom de l'agence à sauvegarder dans office_name__
        # Si parent_name existe et contient " - ", extraire la partie agence
        $validation_office_name = $office_name; // Par défaut
        
        if (!empty($parent_office) && strpos($parent_office, ' - ') !== false) {
            $parts = explode(' - ', $parent_office);
            $validation_office_name = trim($parts[1]); // Prendre la partie agence (ex: "AMPEFILOHA")
        } elseif (!empty($parent_office) && strpos($parent_office, '-') !== false) {
            $parts = explode('-', $parent_office);
            $validation_office_name = trim($parts[1]); // Prendre la partie agence
        }
        ...
        $zone_name = null;
        if ($get_zone_id && isset($get_zone_id->zone_id)) {
            $get_zone_name = DB::table('zones')
                ->select('nom')
                ->where('id', $get_zone_id->zone_id)
                ->first();
            if ($get_zone_name && isset($get_zone_name->nom)) {
        ...
        if (isset($account_subscribed) &&  $account_subscribed->account_status == "1") {
            ...
            try {
                ...
                // Utiliser l'office de l'utilisateur qui fait la demande, pas celui du client dans Musoni
                $validation->office_name = $validation_office_name;
                ...
                $validation->motif_validation = "SOUSCRIPTION";
                $validation->motif = "SOUSCRIPTION"; // Pour compatibilité
                $validation->request_status = "1";
                
                // Ajouter zone si disponible
                if ($zone_name) {
                    $validation->zone = $zone_name;
                }

                $saved = $validation->save();
                
                if (!$saved) {
                    throw new \Exception("Échec de la sauvegarde de la validation");
                }
                ...
                logActivity(
                    session('username'),
                    'subscription',
                    'validation_request_saved_successfully',
                );
                
                return redirect()->route('sub.validation')->with('success', 'Demande de validation envoyée avec succès.');
            } catch (\Exception $e) {
                # log 
                logActivity(
                    session('username'),
                ...
                Log::error('SUBSCRIPTION: Erreur lors de l\'enregistrement de la demande de validation', [
                    'username' => session('username'),
                    'account_no' => $account_no,
                    'ticket' => $ticket,
                    'office_name' => $validation_office_name,
                    'error' => $e->getMessage(),
                    'file' => $e->getFile(),
                    'line' => $e->getLine(),
                ]);
                return redirect()->back()->with('error', 'Erreur lors de l\'enregistrement de la demande : ' . $e->getMessage());
            }
        }
    }
}
